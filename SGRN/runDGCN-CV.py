...
class Encoder(torch.nn.Module):
    def __init__(self, h_sizes):
        ...
        for k in range(self.num_hidden):
            self.hidden.append(DGCNConv(h_sizes[k], h_sizes[k+1], cached=False))
...
test_negIdx = random.sample(list(test_negIdx), opts.kTest*len(test_posIdx))
train_negIdx = random.sample(list(train_negIdx), opts.kTrain*len(train_posIdx))
...
if opts.encoder == 'GCN':
    ...
    data.train_pos_only_edge_index = torch.stack([torch.LongTensor(sourceNodes), torch.LongTensor(targetNodes)], dim=0)
